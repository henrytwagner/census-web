...
export default function ContactSidebar() {
    ...
    // Group by last-name initial (fallback to first-name initial, then '#')
    ...
    return (
        <div className="flex w-50 h-full p-2 flex-col items-start gap-2.5 flex-shrink-0 border-r border-r-border overflow-hidden">
            ...
            <div className="flex flex-col items-start self-stretch overflow-scroll">
                {/* error/empty states stay visually minimal to avoid layout changes */}
                {error ? null : grouped.map(([letter, contacts]) => (
                    <div key={letter} className="contents">
                        {/* GROUP HEADER — identical element & classes */}
                        ...
                        {/* ROWS — identical structure/classes, dynamic names */}
                        {contacts.map((c) => (
                            <div key={c.id} className="flex h-fit px-2 py-3 items-center gap-1.5 self-stretch">
